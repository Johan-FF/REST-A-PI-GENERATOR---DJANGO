...
class NestApiBuilder(Builder):
    ...
    def produce_service(self, entity_name, attributes: list[dict], relations, dependencies):
        ...
        codeService += f"    \"Add-Content -Path '{entity_name.lower()}.service.ts' -Value"+" '                 message: ''Usuario no encontrado'' ';\" ^\n" 
        codeService += f"    \"Add-Content -Path '{entity_name.lower()}.service.ts' -Value"+" '                 }';\" ^\n" 
        codeService += f"    \"Add-Content -Path '{entity_name.lower()}.service.ts' -Value"+" '           }';\" ^\n"
    ...
    def produce_entity(self, entity_name, attributes: list[dict], relations, dependencies):
        ...
        for relation in relations:
            ...
            codeEntity += f"    \"Add-Content -Path '{entity_name_lower}.entity.ts' -Value '    {relation.get("table").lower()}: {relation.get("table").capitalize()};';\" ^\n"
        for dependence in dependencies:
            codeEntity += f"    \"Add-Content -Path '{entity_name_lower}.entity.ts' -Value '    @OneToMany(()"+f" => {dependence.get("table").capitalize()}, ({dependence.get("table").lower()}) => {dependence.get("table").lower()}.{entity_name.lower()})';\" ^\n"
